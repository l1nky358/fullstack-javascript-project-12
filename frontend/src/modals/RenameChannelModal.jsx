...
const RenameChannelModal = ({ channel, onClose }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setStatus }) => {
      ...
      try {
        await dispatch(renameChannel({ id: channel.id, name: values.name.trim() })).unwrap();
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">{t('channels.modals.rename.title', { name: channel.name })}</h5>
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-secondary" onClick={onClose}>
                {t('channels.modals.rename.cancel')}
              </button>
              <button type="submit" className="btn btn-primary" disabled={formik.isSubmitting}>
                {formik.isSubmitting ? t('channels.modals.rename.submitting') : t('channels.modals.rename.submit')}
              </button>
            </div>
          </form>
        </div>
